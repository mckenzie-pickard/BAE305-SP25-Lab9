...
void loop()
{
  ...
  myPID.Compute();
  int speed = map(output, 0, 255, -255, 255);

    if (speed > 0) {
    
      rightMotor(speed); //If you make the left motor positive on both, the robot will do a 360. This would be useful for the wall control distance.
      leftMotor(-speed);
    } else if (speed < 0) {
